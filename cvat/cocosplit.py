...
def main(args):
    with open(args.annotations, 'rt', encoding='UTF-8') as annotations:
        ...
        if args.kfold:
            k = 5
            ss = ShuffleSplit(n_splits=k, test_size=1/k, random_state=0)
            for (train_index, test_index),j in zip(ss.split(images),range(k)):
                x = [images[i] for i in train_index]
                y = [images[i] for i in test_index]
                save_coco(args.train + str(j) + '.json', info, licenses, x, filter_annotations(annotations, x),
                          categories)
                save_coco(args.test + str(j) + '.json', info, licenses, y, filter_annotations(annotations, y),
                          categories)
        ...
        if args.shuffle:
            ...
            x, y = train_test_split(images, train_size=args.split, shuffle=False)
